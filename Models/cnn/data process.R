...
# - each row = one ping
# - split by track_id to avoid leakage
# =========================================================
...
# Focus only on 70 kHz as Marco suggested
# -------------------------
...
# Keep track_id so we can split by track later
# -------------------------
data_keep <- full_data %>%
  ...
  pivot_wider(
    id_cols = c(track_id, Ping_index, Region_name, Year, Month, kHz, Location, species),
    names_from = variable,
    values_from = value
  )
...
# -------------------------
# Optional check:
# frequency spacing is already equal,
# so interpolation is unnecessary
# -------------------------
freq_numeric <- as.numeric(sub("^F", "", freq_cols))
cat("\nDifferences between consecutive frequencies:\n")
print(unique(diff(sort(freq_numeric))))

# -------------------------
# Track-level split to avoid leakage
# All pings from the same track stay in the same set
# -------------------------
track_level <- model_df_complete %>%
  select(track_id, species) %>%
  distinct()

set.seed(15)
train_index <- createDataPartition(track_level$species, p = 0.7, list = FALSE)

train_tracks <- track_level[train_index, ]
temp_tracks  <- track_level[-train_index, ]

set.seed(15)
val_index <- createDataPartition(temp_tracks$species, p = 0.5, list = FALSE)

validate_tracks <- temp_tracks[val_index, ]
test_tracks     <- temp_tracks[-val_index, ]

train_df <- model_df_complete %>%
  filter(track_id %in% train_tracks$track_id)

validate_df <- model_df_complete %>%
  filter(track_id %in% validate_tracks$track_id)

test_df <- model_df_complete %>%
  filter(track_id %in% test_tracks$track_id)

cat("\nTrack overlap check:\n")
cat("train ∩ validation =", length(intersect(unique(train_df$track_id), unique(validate_df$track_id))), "\n")
cat("train ∩ test =", length(intersect(unique(train_df$track_id), unique(test_df$track_id))), "\n")
cat("validation ∩ test =", length(intersect(unique(validate_df$track_id), unique(test_df$track_id))), "\n")
...
cat("\nNumber of tracks in each split:\n")
print(c(
  train_tracks = length(unique(train_df$track_id)),
  validation_tracks = length(unique(validate_df$track_id)),
  test_tracks = length(unique(test_df$track_id))
))
...
# -------------------------
# Month metadata
# Keep if you still want month as an extra input
# -------------------------
