...

{
  ...
  mat_final_data = c(val_loss = 999, best_epoch_loss = -1 ,val_auc = -1, best_val_acc = -1, best_val_auc = -1,model_id = -1)
  t1 = Sys.time()
  
  for(i in batch_fitted_models){
    model_fitted = file.exists(paste0("Models/dnn/drac/training/training_metrics_full/training_metrics_b",nbatch,"/training_output_",i,".rds"))
    if(model_fitted){
      aux_row = readRDS(paste0("Models/dnn/drac/training/training_metrics_full/training_metrics_b",nbatch,"/training_output_",i,".rds"))
...
saveRDS(final_data,file = paste0("Models/dnn/drac/training/training_metrics_full/val_metrics_b",nbatch,".rds"))
